...
// import { MagistrateContext,  courtDivision } from "@/context/MagistrateContext";
...
import { ROLES } from "@/types/auth";
import Pagination from "@/components/ui/pagination";
import { MagistrateContext } from "@/context/MagistrateContext"; // ✅ Fixed import
...
export default function MagistratesTable() {
  ...
  const courtDivision = magistrateContext?.courtDivision || ""; // ✅ Retrieve `courtDivision` from context
  ...
  const { data, isLoading: draftsLoading } = useQuery({
    queryKey: ["magistrate-oversight", { searchQuery, courtDivision, currentPage, selectedCourt, role, division_id: user?.court_division_id }], // ✅ Fixed `courtDivision` access
    queryFn: async () =>
      await getOversight({
        page: currentPage,
        ...
        search: searchQuery,
        usertype: role,
        casetype: selectedCourt === "all" ? "" : selectedCourt, // ✅ Ensuring "all" is converted to an empty string
      }),
    staleTime: 50000,
  });
...
//   const { data, isLoading: draftsLoading } = useQuery({
//     queryKey: ["magistrate-oversight", { searchQuery, courtDivision, currentPage, selectedCourt, role, division_id: user?.court_division_id }],
//     queryFn: async () =>
//       await getOversight({
//         page: currentPage,
//         size: DEFAULT_PAGE_SIZE,

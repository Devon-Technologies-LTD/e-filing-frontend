...
export const LoginIdField = (props:
    ...
    return (
        <div>
            <Label htmlFor="staff_id" className="text-xs text-muted-foreground">{props.field_label ? props.field_label : 'Staff identification'}</Label>
            <Input
                ...
                name={props.field_name ? props.field_name : 'staff_id'} id={props.field_id ? props.field_id : 'staff_id'}
...
export const LoginPasswordField = (props: { state?: { status: any; message: any; errors: any; } }) => {
    ...
    return (
        <div>
            ...
            <span className="h-1 text-xs text-destructive">
                {props.state?.errors && props.state.errors.password}
            </span>
        </div>
    )
}
